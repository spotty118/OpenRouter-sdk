...
async function runExample() {
    ...
    const weatherFunction = orchestrator.registerFunction('get_weather', 'Get current weather for a location', {
        location: {
            type: 'string',
    ...
    const searchFunction = orchestrator.registerFunction('search_web', 'Search the web for information', {
        query: {
            type: 'string',
    ...
    const searchResults = await orchestrator.searchByText('research-knowledge', 'electric vehicle market growth', { limit: 5, minScore: 0.7 });
