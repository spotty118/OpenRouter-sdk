...
class OpenRouter {
    ...
    async createChatCompletion(options) {
        ...
        try {
            const response = await this.makeRequest('POST', url, modifiedPayload);
            // Track cost
            if (response.usage) {
                const model = await this.getModelInfo(modifiedPayload.model || this.defaultModel);
                if (model) {
                    const costEstimate = this.estimateCost(model, response.usage.prompt_tokens, response.usage.completion_tokens);
    ...
    async *streamChatCompletions(options) {
        ...
        try {
            ...
            if (modelInfo) {
                const costEstimate = this.estimateCost(modelInfo, promptTokens, completionTokens);
                this.totalCost += costEstimate.totalCost;
            }
        }
        catch (error) {
    ...
    async createEmbedding(options) {
        ...
        return await this.makeRequest('POST', url, options);
    }
    /**
    ...
    async listModels() {
        ...
        try {
            ...
            const models = await this.makeRequest('GET', url);
            // Cache the result
     ...
     * ```
     */
    async batchChatCompletions(requests, concurrency = 3) {
     ...
     * ```
     */
    withFallbacks(primaryModel, fallbackModels) {
    ...
    createAgent(agentConfig) {
        return this.crewAI.createAgent(agentConfig);
    }
    /**
     ...
     * ```
     */
    async getAgentKnowledgeDocument(agentId, documentId, namespace) {
     ...
     * ```
     */
    async deleteAgentKnowledgeDocument(agentId, documentId, namespace) {
     ...
     */
    estimateCost(model, promptTokens, completionTokens = 0) {
        ...
        this.logger.debug(`Cost estimate for ${model.id}:`, { promptTokens, completionTokens, promptCost, completionCost, totalCost });
        return {
            promptCost,
            completionCost,
            totalCost,
